...
import {
  LayerMaterial, Base, Depth, Fresnel, Noise,
} from 'lamina/vanilla';
...
const material = new LayerMaterial({
  layers: [
    ...
    new Depth({
      colorA, colorB, alpha: 0.5, mode: 'normal', near: 0, far: 2, origin: [1, 1, 1],
    }),
    new Depth({
      colorA: 'purple', colorB, alpha: 0.5, mode: 'add', near: 3, far: 2, origin: [1, 1, 1],
    }),
    new Fresnel({
      mode: 'add', color: fresnel, intensity: 0.3, power: 2.5, bias: 0.0,
    }),
    new Noise({
      mapping: 'local', type: 'simplex', scale: 1000, colorA: '#ffaf40', colorB: 'black', mode: 'overlay',
    }),
  ],
});
...
function Noodle() {
  ...
  const position = useMemo(() => {
    ...
    return [THREE.MathUtils.randFloatSpread(bounds.width),
      THREE.MathUtils.randFloatSpread(bounds.height * 0.75), z];
  }, [viewport]);
  return (
    <Float
      position={position}
